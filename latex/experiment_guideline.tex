...
artifact selection or staging. It then commits only that job's exact
stdout/stderr pair and launch-tagged run/report roots, excluding \code{*.pt},
\code{*.npy}, and \code{*.cbm}, and pushes \code{origin/main} without opening a
pull request. Omitting the job ID publishes all logs and lightweight outputs; a
