...
\code{*.cbm}, then sources \code{$HOME/codes/proxy.sh} with the shared two-line
\code{$HOME/codes/.secrets/proxy.credentials} and pushes \code{origin/main}. It never pulls
or opens a pull request. \code{PUBLISH_RESULTS=false} disables submission; the
publisher's \code{--job-id} option supports manual retry.
