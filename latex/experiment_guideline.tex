...
and has status \code{not_run}, \code{running}, \code{ready}, \code{interrupted}, or
\code{completed}. Run identity contains only the schema, identity/model configs,
...
changed pipeline. Finished seeds and runs remain \code{ready}; overall completion is
written only on the owning Slurm workflow's final successful exit. Completed manifests
...
live at \code{outputs/reports/family/mode} and record requested filters and
obtained input manifests.

After successful completion, the workflow submits \code{publish.slurm} with an
\code{afterok} dependency. It commits only the producer's exact stdout/stderr
and launch-tagged run/report roots, excluding \code{*.pt}, \code{*.npy}, and
\code{*.cbm}, then sources \path{$HOME/codes/proxy.sh} with the shared two-line
\path{$HOME/codes/.secrets/proxy.credentials} and pushes \code{origin/main}. It never pulls
or opens a pull request. A repository lock serializes the complete Git
transaction and waits up to ten minutes.
\code{PUBLISH_RESULTS=false} disables submission; the publisher's \code{--job-id}
option supports manual retry.

Ninety-four reconstructable historical configurations were migrated. Their
source trees remain under
\path{outputs/archive/legacy_pre_schema_v1_2026-08-07}; current readers exclude
that archive.
