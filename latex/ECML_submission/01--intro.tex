...
The RevIN method has been adopted in numerous recent forecasting studies \citep{patchtst,fits,tirex,HUANG2025114036,revin}. Its authors argue that RevIN mitigates the distribution shift problem in time series. In this work, we investigate this claim more closely and ask when its inductive bias is preferable to global standardization.
Our contributions are summarized as follows:
\begin{itemize}
    \item We separate global standardization, per-window normalization, RevIN's affine parameters, and normalized-space backpropagation under a common forecasting protocol.
    \item We evaluate generalization to future dates and unseen series across datasets, look-back/horizon settings, two backbones, and multiple seeds, reporting both mean error and uncertainty.
    \item We characterize the trade-off that determines when instance normalization helps: removing nuisance offset and scale heterogeneity versus preserving predictive absolute-level information. We also quantify the optimistic headroom of choosing standard or instance normalization per setting.
\end{itemize}
