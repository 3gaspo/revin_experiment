\begin{table}[t]
\caption{Previous-implementation reproduction targets for \textsc{PatchTST}
(MSE on new dates; means only). BP stands for ``backpropagation'' and Std. for
``standard''. New runs replace these values with mean $\pm$ seed standard
deviation.}
\centering
...
\begin{table}[t]
\caption{Previous-implementation reproduction targets for \textsc{PatchTST}
(MSE on new dates and new users; means only). BP stands for ``backpropagation''
and Std. for ``standard''. New runs replace these values with mean $\pm$ seed
standard deviation.}
\centering
\setlength{\tabcolsep}{1.5pt}
\begin{tabular}{lccccccc}
